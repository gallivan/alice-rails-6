RSpec.describe Account, type: :model do
  ...
  describe "#handle_fill CME:ESU16:01974:21937832663 - one day one fill" do
    ...
    before(:all) do
      ...
      # <FIXML>
      #   <TrdCaptRpt RptID="21964458435" TrdTyp="0" ExecID="27712820160725115508TN0276673" TrdDt="2016-07-25"
      #               BizDt="2016-07-25"
      #               MLegRptTyp="1" MtchStat="0" MsgEvtSrc="REG" TrdID="106066" LastQty="2" LastPx="2158.75"
      #               TxnTm="2016-07-25T11:55:08-05:00" SettlCcy="USD" SettlDt="2016-09-16"
      #               OrigTrdID="1561EEC09E5LTP0301D262D90"
      #               PxSubTyp="1" VenueTyp="E" VenuTyp="E">
      #     <Instrmt ID="ES" Desc="E-MINI S&amp;P 500 FUTURES" CFI="FFICSO" SecTyp="FUT" MMY="201609" MatDt="2016-09-16"
      #              Mult="50"
      #              Exch="CME" UOM="IPNT" UOMQty="50" PxUOM="IPNT" PxUOMQty="1" ValMeth="FUT" Fctr="1"
      #              PxQteCcy="USD"></Instrmt>
      #     <Amt Typ="TVAR" Amt="345" Ccy="USD"/>
      #     <RptSide Side="1" ClOrdID="EZLZ" CustCpcty="2" OrdTyp="L" SesID="EOD" SesSub="E" AllocInd="0" AgrsrInd="N">
      #       <Pty ID="CME" R="21"></Pty>
      #       <Pty ID="353" R="4"></Pty>
      #       <Pty ID="CME" R="22"></Pty>
      #       <Pty ID="353" R="1"></Pty>
      ...
      #       </Pty>
      #       <Pty ID="353" R="7"></Pty>
      #       <RegTrdID ID="FECF1561EEC09E5LTP0301D262D92" Src="1010000023" Typ="0" Evnt="2"/>
      #     </RptSide>
      #   </TrdCaptRpt>
      # </FIXML>

      # get execution report
      name = "#{Rails.root}/etc/data/cme/rspec/CME:ESU16:01974:2016-07-25:21964458435.xml"
    ...
    context "when account 01974 executes CME:ESU16 via CME EOD with id 21959018827" do
      it "then account with code '01974' should exist" do
        expect(@account.code).to eql('01974')
      end
      ...
      it "and the fill has appropriate attributes" do
        ...
        expect(@account.deal_leg_fills.last.dealing_venue_done_id).to eq('27712820160725115508TN0276673 106066')
        expect(@account.deal_leg_fills.last.claim.code).to eq('CME:ESU16')
        expect(@account.deal_leg_fills.last.done).to eq(+2)
        expect(@account.deal_leg_fills.last.price).to eq(2158.75)
        expect(@account.deal_leg_fills.last.price_traded).to eq('2158.75')
      ...
      it "and the position has appropriate attributes" do
        ...
        expect(@account.positions.last.price).to eq(2158.75)
        expect(@account.positions.last.price_traded).to eq('2158.75')
        expect(@account.positions.last.mark).to eq(2158.55)
        expect(@account.positions.last.bot).to eq(+2)
        ...
        expect(@account.positions.last.net).to eq(+2)
      ...
      it "and the statement position has appropriate attributes" do
        ...
        expect(@account.statement_positions.last.price).to eq(2158.75)
        expect(@account.statement_positions.last.mark).to eq(2158.55)
        expect(@account.statement_positions.last.bot).to eq(2)
        expect(@account.statement_positions.last.sld).to eq(0)
        expect(@account.statement_positions.last.net).to eq(+2)
      ...
      it "and appropriate USD SEGN money line values exist" do
        ...
        expect(money_line.ledger_balance).to eq(BigDecimal(-98.40, 8))
        expect(money_line.open_trade_equity).to eq(-20.00)
        expect(money_line.cash_account_balance).to eq(BigDecimal(-118.40, 8))
        ...
        expect(money_line.net_liquidating_balance).to eq(BigDecimal(-118.40, 8))
      end

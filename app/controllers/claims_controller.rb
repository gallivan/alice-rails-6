class ClaimsController < ApplicationController
  ...
  # GET /claims.json
  def index
    @q = Claim.order(:code).ransack(params[:q])
    @claims = @q.result.page(params[:page]).per(20)
  end
  ...
  private
    ...
    def claim_params
      params.require(:claim).permit(:claim_set_id, :claim_type_id, :entity_id, :claimable_id, :claimable_type, :code, :name, :size, :point_value, :point_currency_id)
    end
end

...
ActiveRecord::Schema.define(version: 2019_07_26_230250) do
  ...
  create_table "money_lines", id: :serial, force: :cascade do |t|
    t.integer "account_id", null: false
    t.integer "currency_id", null: false
    t.date "posted_on", null: false
  ...
  create_view "abs_done_by_account_and_claim_set_view",  sql_definition: <<-SQL
      SELECT t.posted_on,
      a.code AS account_code,
      s.code AS claim_set_code,
      (sum(abs(t.done)))::integer AS done
     FROM deal_leg_fills t,
      claim_sets s,
      accounts a,
      claims c
    WHERE ((a.id = t.account_id) AND (c.id = t.claim_id) AND (s.id = c.claim_set_id))
    GROUP BY t.posted_on, a.code, s.code;
  SQL

  create_view "abs_done_by_claim_set_view",  sql_definition: <<-SQL
      SELECT t.posted_on,
      s.code AS claim_set_code,
      (sum(abs(t.done)))::integer AS done
     FROM deal_leg_fills t,
      claim_sets s,
      claims c
    WHERE ((c.id = t.claim_id) AND (s.id = c.claim_set_id))
    GROUP BY t.posted_on, s.code;
  SQL

  create_view "ccy_otes",  sql_definition: <<-SQL
      SELECT z.posted_on,
      z.account_code,
      z.claim_set_code,
      sum(abs(z.net)) AS gross,
      (sum(z.ote))::numeric(16,2) AS ccy_ote,
      z.currency_code
     FROM ( SELECT p.posted_on,
              a.code AS account_code,
              s.code AS claim_set_code,
              p.net,
              p.ote,
             ...
             FROM claims c,
              ...
              positions p,
              position_statuses t
            WHERE ((t.id = p.position_status_id) AND ((t.code)::text = 'OPN'::text) AND (c.id = p.claim_id) AND (a.id = p.account_id) AND (x.id = c.point_currency_id) AND (s.id = c.claim_set_id) AND (x.id = m.currency_id) AND (p.posted_on = m.posted_on))) z
    GROUP BY z.posted_on, z.account_code, z.claim_set_code, z.currency_code;
  SQL

  create_view "charges_by_exchange_and_posted_on_view",  sql_definition: <<-SQL
      SELECT f.posted_on,
     ...
     FROM entities e,
      ...
      chargeables c,
      chargeable_types t
    WHERE ((s.id = c.claim_set_id) AND (x.id = c.currency_id) AND (c.id = f.chargeable_id) AND (t.id = c.chargeable_type_id) AND ((e.code)::text = split_part((s.code)::text, ':'::text, 1)))
    GROUP BY f.posted_on, t.id, x.id, e.id
    ORDER BY f.posted_on DESC, e.code, t.code, x.code;
  SQL

  create_view "claim_futures_view",  sql_definition: <<-SQL
     ...
     FROM claims c,
      futures f
    WHERE (f.id = c.claimable_id);
  SQL

  create_view "exp_held_view",  sql_definition: <<-SQL
      SELECT x.posted_on,
      x.account_code,
      x.claim_set_code,
      (sum(x.exp))::numeric(16,2) AS sum,
      x.currency_code
     FROM ( SELECT c.posted_on,
              a.code AS account_code,
              s.code AS claim_set_code,
              c.amount,
              x_1.code,
              c.amount AS exp,
              x_1.code AS currency_code
             FROM accounts a,
              charges c,
              chargeables b,
              claim_sets s,
              currencies x_1
            WHERE ((a.id = c.account_id) AND (b.id = c.chargeable_id) AND (s.id = b.claim_set_id) AND (x_1.id = c.currency_id))) x
    GROUP BY x.posted_on, x.account_code, x.claim_set_code, x.currency_code;
  SQL

  create_view "exp_view",  sql_definition: <<-SQL
      SELECT x.posted_on,
      x.account_code,
      x.claim_set_code,
      (sum(x.exp_usd))::numeric(16,2) AS exp_usd
     FROM ( SELECT c.posted_on,
              a.code AS account_code,
              s.code AS claim_set_code,
              c.amount,
              x_1.code,
                  CASE
                      WHEN ((x_1.code)::text = ANY (ARRAY[('EUR'::character varying)::text, ('GBP'::character varying)::text, ('AUD'::character varying)::text, ('NZD'::character varying)::text])) THEN (c.amount * m.mark)
             ...
             FROM accounts a,
              ...
              currencies x_1,
              currency_marks m
            WHERE ((a.id = c.account_id) AND (b.id = c.chargeable_id) AND (s.id = b.claim_set_id) AND (x_1.id = c.currency_id) AND (x_1.id = m.currency_id) AND (c.posted_on = m.posted_on))) x
    GROUP BY x.posted_on, x.account_code, x.claim_set_code;
  SQL

  create_view "futures_profit_and_loss_by_currency_and_posted_on_view",  sql_definition: <<-SQL
     ...
     FROM currencies c,
      money_lines m
    WHERE (((m.kind)::text = 'HELD'::text) AND (c.id = m.currency_id))
    GROUP BY c.id, m.kind, m.posted_on
    ORDER BY m.posted_on DESC, c.code;
  SQL

  create_view "held_by_exg_ccy_otes",  sql_definition: <<-SQL
      SELECT ccy_otes.posted_on,
      split_part((ccy_otes.claim_set_code)::text, ':'::text, 1) AS exg_code,
      sum(ccy_otes.ccy_ote) AS ccy_ote,
      ccy_otes.currency_code
     FROM ccy_otes
    GROUP BY ccy_otes.posted_on, (split_part((ccy_otes.claim_set_code)::text, ':'::text, 1)), ccy_otes.currency_code;
  SQL

  create_view "inc_held_view",  sql_definition: <<-SQL
      SELECT x.posted_on,
      x.account_code,
      x.claim_set_code,
      sum(x.done) AS done,
      (sum(x.pnl))::numeric(16,2) AS inc,
      x.currency_code
     FROM ( SELECT n.posted_on,
              a.code AS account_code,
              s.code AS claim_set_code,
              n.done,
              n.pnl,
             ...
             FROM claims c,
              ...
              currencies x_1,
              position_nettings n
            WHERE ((c.id = n.claim_id) AND (a.id = n.account_id) AND (x_1.id = c.point_currency_id) AND (s.id = c.claim_set_id))) x
    GROUP BY x.posted_on, x.account_code, x.claim_set_code, x.currency_code;
  SQL

  create_view "inc_by_exg_held",  sql_definition: <<-SQL
      SELECT h.posted_on,
      split_part((h.claim_set_code)::text, ':'::text, 1) AS exg_code,
      sum(h.inc) AS inc,
      h.currency_code
     FROM inc_held_view h
    GROUP BY h.posted_on, (split_part((h.claim_set_code)::text, ':'::text, 1)), h.currency_code;
  SQL

  create_view "inc_view",  sql_definition: <<-SQL
      SELECT x.posted_on,
      x.account_code,
      x.claim_set_code,
      sum(x.done) AS done,
      (sum(x.pnl_usd))::numeric(16,2) AS inc_usd
     FROM ( SELECT n.posted_on,
              a.code AS account_code,
              s.code AS claim_set_code,
              n.done,
                  CASE
                      WHEN ((x_1.code)::text = ANY (ARRAY[('EUR'::character varying)::text, ('GBP'::character varying)::text, ('AUD'::character varying)::text, ('NZD'::character varying)::text])) THEN (n.pnl * m.mark)
                      ELSE (n.pnl / m.mark)
                  END AS pnl_usd
             FROM claims c,
              claim_sets s,
              ...
              currency_marks m,
              position_nettings n
            WHERE ((c.id = n.claim_id) AND (a.id = n.account_id) AND (x_1.id = c.point_currency_id) AND (s.id = c.claim_set_id) AND (x_1.id = m.currency_id) AND (n.posted_on = m.posted_on))) x
    GROUP BY x.posted_on, x.account_code, x.claim_set_code;
  SQL

  create_view "inc_exp_usds",  sql_definition: <<-SQL
      SELECT i.posted_on,
      i.account_code,
      i.claim_set_code,
     ...
     FROM inc_view i,
      exp_view e,
      claim_sets s
    WHERE ((i.posted_on = e.posted_on) AND ((i.account_code)::text = (e.account_code)::text) AND ((i.claim_set_code)::text = (e.claim_set_code)::text) AND ((e.claim_set_code)::text = (s.code)::text))
    ORDER BY i.posted_on, i.account_code, i.claim_set_code;
  SQL

  create_view "ledger_balance_by_currency_and_posted_on_view",  sql_definition: <<-SQL
      SELECT c.id AS currency_id,
      m.posted_on,
  ...
  create_view "net_charged_by_account_and_claim_set_view",  sql_definition: <<-SQL
      SELECT x.posted_on,
      a.code AS account_code,
      s.code AS claim_set_code,
     ...
     FROM chargeable_types t,
      chargeables c,
      claim_sets s,
      currencies f,
      accounts a,
      charges x
    WHERE ((t.id = c.chargeable_type_id) AND (c.id = x.chargeable_id) AND (s.id = c.claim_set_id) AND (f.id = c.currency_id) AND (a.id = x.account_id))
    GROUP BY x.posted_on, a.code, f.code, s.code, t.code
    ORDER BY x.posted_on, a.code, s.code, t.code, f.code;
  SQL

  create_view "net_charged_by_claim_set_view",  sql_definition: <<-SQL
      SELECT x.posted_on,
      s.code AS claim_set_code,
      t.code AS chargeable_type_code,
      f.code AS currency_code,
     ...
     FROM chargeable_types t,
      ...
      currencies f,
      charges x
    WHERE ((t.id = c.chargeable_type_id) AND (c.id = x.chargeable_id) AND (s.id = c.claim_set_id) AND (f.id = c.currency_id))
    GROUP BY x.posted_on, f.code, s.code, t.code
    ORDER BY x.posted_on, s.code, t.code, f.code;
  SQL

  create_view "open_trade_equity_by_currency_and_posted_on_view",  sql_definition: <<-SQL
      SELECT c.id AS currency_id,
      m.posted_on,
  ...
  create_view "statement_position_by_claim_and_account_view",  sql_definition: <<-SQL
      ...
      x.sld,
      x.net,
      round((x.price / x.net), 6) AS price,
      round((x.mark / x.net), 6) AS mark,
      round(x.ote, 2) AS ote
     FROM ( SELECT f.id AS currency_id,
              a.id AS account_id,
              c.id AS claim_id,
              p.account_code,
              p.claim_code,
              ...
              sum(p.sld) AS sld,
              sum(p.net) AS net,
              sum((p.net * p.price)) AS price,
              sum((p.net * p.mark)) AS mark,
              sum(p.ote) AS ote
             FROM currencies f,
              accounts a,
              claims c,
              statement_positions p
            WHERE (((f.code)::text = (p.currency_code)::text) AND ((a.code)::text = (p.account_code)::text) AND ((c.code)::text = (p.claim_code)::text))
            GROUP BY f.id, a.id, c.id, p.account_code, p.claim_code, p.stated_on
            ORDER BY p.stated_on DESC, p.account_code, p.claim_code) x;
  SQL

  create_view "statement_position_by_claim_view",  sql_definition: <<-SQL
      SELECT x.currency_id,
      x.claim_id,
      x.stated_on,
      x.bot,
      ...
      x.avg_sld_px,
          CASE
              WHEN ((x.bot > (0)::numeric) AND (x.sld > (0)::numeric)) THEN round(((x.avg_bot_px + x.avg_sld_px) / (2)::numeric), 6)
              ELSE (x.avg_bot_px + x.avg_sld_px)
          END AS price,
      x.mark,
      round(x.ote, 2) AS ote
     FROM ( SELECT f.id AS currency_id,
              ...
              sum(p.net) AS net,
                  CASE
                      WHEN (sum(p.bot) > (0)::numeric) THEN round((sum((p.bot * p.price)) / sum(p.bot)), 6)
                      ELSE (0)::numeric
                  END AS avg_bot_px,
                  CASE
                      WHEN (sum(p.sld) > (0)::numeric) THEN round((sum((p.sld * p.price)) / sum(p.sld)), 6)
                      ELSE (0)::numeric
                  END AS avg_sld_px,
              p.mark,
              sum(p.ote) AS ote
             FROM claims c,
              currencies f,
              statement_positions p
            WHERE (((f.code)::text = (p.currency_code)::text) AND ((c.code)::text = (p.claim_code)::text))
            GROUP BY c.id, f.id, p.claim_code, p.stated_on, p.mark
            ORDER BY p.stated_on DESC, p.claim_code) x;
  SQL

  create_view "usd_otes",  sql_definition: <<-SQL
      SELECT x.posted_on,
      x.account_code,
      x.claim_set_code,
      sum(x.net) AS net,
     ...
     FROM ( SELECT p.posted_on,
              ...
              p.net,
                  CASE
                      WHEN ((x_1.code)::text = ANY (ARRAY[('EUR'::character varying)::text, ('GBP'::character varying)::text, ('AUD'::character varying)::text, ('NZD'::character varying)::text])) THEN (p.ote * m.mark)
                      ELSE (p.ote / m.mark)
                  END AS ote_usd
             FROM claims c,
              claim_sets s,
              ...
              positions p,
              position_statuses t
            WHERE ((t.id = p.position_status_id) AND ((t.code)::text = 'OPN'::text) AND (c.id = p.claim_id) AND (a.id = p.account_id) AND (x_1.id = c.point_currency_id) AND (s.id = c.claim_set_id) AND (x_1.id = m.currency_id) AND (p.posted_on = m.posted_on))) x
    GROUP BY x.posted_on, x.account_code, x.claim_set_code;
  SQL

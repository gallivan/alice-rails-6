def build_ledger_entry(line, index, code, posted_on, account, ccy)
  ary = line.split(/\s+/)
  val = ary[index].gsub(',', '')
...
task :seed_balances, [:filename] => :environment do |t, args|
  ...
  File.open(filename).readlines.each do |line|
    ...
    if line.match(/^(USD|EUR|GBP|CHF|JPY|AUD|NZD)/)
      ...
      leg = build_ledger_entry(line, 8, 'LEG', posted_on, account, ccy)
    end

    if line.match(/^\s{27}[\*]{20}/)
      ote = build_ledger_entry(line, 3, 'OTE', posted_on, account, ccy)
    end
